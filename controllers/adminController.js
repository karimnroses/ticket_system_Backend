...
export const createNewCompany = async (req, res) => {
  ...
  const { company_name, adress, number, zip, city, country, phone, email } =
    req.body;
  await pool
    .query(
      `
...
/*********************___Update User Infos Inkl Passwort___*************************/
export const updateUserInfos = async (req, res) => {
 ...
 console.log(first_name, last_name, email, company_id, role_id, user_id);
  await pool.query(
    `
    UPDATE users u
    SET
    ...
    role_id = $6,
    status_id = $7
    WHERE u.id = $8 RETURNING *;
    `, [first_name, last_name, username, email, company_id, role_id, status_id, user_id]
    )
    .then((results) => {
      res.status(201).json(results);
    })
